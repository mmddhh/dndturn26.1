...
Build and inspect one target's jar, then publish from its own directory with the JDK declared in its `ci.properties`:

```powershell
cd targets\fabric-1.20.1
.\gradlew.bat clean build
# Inspect the target jar and metadata before publishing.
.\gradlew.bat publish
```
