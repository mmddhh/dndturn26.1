...
- Java 17 兼容的业务逻辑、数学、状态机、队列和不可变 DTO。
...
- 多一两步：Forge target 提供 hook；其余 target 显式声明不支持该能力，或使用符合共同合约的默认行为。
...
`.github/workflows/verify-common.yml` 是 `common` 的 CI 门禁。push / pull request 按以下路径过滤，手动触发不受路径过滤限制；矩阵从 `targets/*/ci.properties` 动态发现，具体规则见 [CI target 发现](CI_TARGET_DISCOVERY.md)：
...
2. 动态发现的启用 target job 并行执行各自 wrapper 的 `clean build`。`build` 包含 target 和 common 的测试任务。
3. 每个 target 使用 `ci.properties` 声明的 JDK，与上表和实际工具链保持一致。
4. 成功或失败时上传已生成的 `build/libs/*.jar`，便于检查发布物。

`scripts/build-target.ps1` 默认最多尝试三次，`ci.attempts` 可设为 1–5。脚本对任意非零退出码重试，不区分网络、编译或测试失败；达到尝试上限仍失败时使 job 失败。
...
```text
DNDTurn-forge-1.20.1-<version>.jar
DNDTurn-fabric-1.20.1-<version>.jar
DNDTurn-neoforge-1.21.1-<version>.jar
DNDTurn-neoforge-26.1-<version>.jar
```
