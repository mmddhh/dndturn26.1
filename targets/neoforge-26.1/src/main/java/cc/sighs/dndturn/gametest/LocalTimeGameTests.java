...
public final class LocalTimeGameTests {
    ...
    public static final DeferredHolder<Consumer<GameTestHelper>, Consumer<GameTestHelper>> LOCAL_TIME =
        TEST_FUNCTIONS.register("local_time", () -> LocalTimeGameTests::localTime);
    ...
    private static void canceledDeath(GameTestHelper helper) {
        ...
        MinecraftCombatRuntime.beginLocal(helper.getLevel().getServer(), Set.of(zombie.getUUID()));
        helper.assertTrue(MinecraftCombatRuntime.isLocalMember(helper.getLevel().getServer(), zombie.getUUID()),
            "local death regression did not acquire the member");
        zombie.kill(helper.getLevel());
        helper.runAtTickTime(4, () -> {
            helper.assertFalse(MinecraftCombatRuntime.isLocalMember(helper.getLevel().getServer(), zombie.getUUID()),
                "confirmed local death retained body control");
            helper.succeed();
        });
    }
    ...
    private static void encounterService(GameTestHelper helper) {
        ...
        boolean localRejected = false;
        try {
            MinecraftCombatRuntime.beginLocal(helper.getLevel().getServer(), Set.of(player.getUUID()));
        } catch (IllegalStateException expected) {
            localRejected = true;
        }
        helper.assertTrue(localRejected, "Debug local controller acquired a formal member");
        UUID operationId = UUID.randomUUID();
    ...
    private static void localTime(GameTestHelper helper) {
        var level = helper.getLevel();
        var server = level.getServer();
        ServerPlayer player = helper.makeMockServerPlayerInLevel();
        player.connection.markClientLoaded();
        ArmorStand outside = helper.spawn(EntityType.ARMOR_STAND, new BlockPos(2, 41, 2));
        outside.setNoGravity(true);
        player.addEffect(new MobEffectInstance(MobEffects.SPEED, 100));
        int initialDuration = player.getEffect(MobEffects.SPEED).getDuration();
        int outsideTicks = outside.tickCount;
        long initialWorldTime = level.getGameTime();
        UUID member = player.getUUID();
        MinecraftCombatRuntime.beginLocal(server, Set.of(member));

        helper.runAtTickTime(8, () -> {
            helper.assertTrue(MinecraftCombatRuntime.isBodyPaused(player), "Player body was not paused");
            helper.assertTrue(MinecraftCombatRuntime.localControllerTicks(server, member) >= 7,
                "Virtual controller did not tick");
            helper.assertTrue(level.getGameTime() > initialWorldTime, "World time stopped globally");
            helper.assertTrue(outside.tickCount > outsideTicks, "Unrelated entity stopped ticking");
            helper.assertTrue(player.getEffect(MobEffects.SPEED).getDuration() == initialDuration,
                "Connection-driven player effects advanced while body was paused");
            player.connection.tick();
            helper.assertTrue(player.getEffect(MobEffects.SPEED).getDuration() == initialDuration,
                "Paused player connection advanced the body");
            float originalYaw = player.getYRot();
            player.connection.handleMovePlayer(new ServerboundMovePlayerPacket.Rot(originalYaw + 45.0F, 0.0F, true, false));
            helper.assertTrue(player.getYRot() == originalYaw, "Paused player accepted a movement packet");
            MinecraftCombatRuntime.authorizeLocalBodyTicks(server, member, 3);
            player.connection.tick();
            helper.assertTrue(player.getEffect(MobEffects.SPEED).getDuration() < initialDuration,
                "Authorized player connection did not run vanilla body tick");
            player.connection.handleMovePlayer(new ServerboundMovePlayerPacket.Rot(originalYaw + 45.0F, 0.0F, true, false));
            helper.assertTrue(player.getYRot() != originalYaw, "Authorized player input never reached vanilla movement handling");
        });

        helper.runAtTickTime(13, () -> {
            helper.assertTrue(MinecraftCombatRuntime.isBodyPaused(player), "Action window did not expire");
            helper.assertTrue(player.getEffect(MobEffects.SPEED).getDuration() < initialDuration,
                "Authorized body never resumed the vanilla player tick");
            int durationAfterAction = player.getEffect(MobEffects.SPEED).getDuration();
            player.connection.tick();
            helper.assertTrue(player.getEffect(MobEffects.SPEED).getDuration() == durationAfterAction,
                "Expired action window continued to advance the body");
            MinecraftCombatRuntime.endLocal(server, member);
            player.connection.tick();
            helper.assertTrue(player.getEffect(MobEffects.SPEED).getDuration() < durationAfterAction,
                "Released body did not resume the vanilla player tick");
            helper.assertFalse(MinecraftCombatRuntime.isLocalMember(server, member), "Encounter retained its player");
            helper.succeed();
        });
    }
}
