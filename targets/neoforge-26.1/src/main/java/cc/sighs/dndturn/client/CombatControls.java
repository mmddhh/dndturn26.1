...
/** Rebindable tactical controls. Packets carry intent, never authoritative costs or damage. */
public final class CombatControls {
    ...
    private static void renderHud(GuiGraphicsExtractor graphics, net.minecraft.client.DeltaTracker delta) {
        var state = ClientCombatState.encounter();
        Minecraft game = Minecraft.getInstance();
        if (state == null || game.player == null || TacticalOverlay.isOpen() || !TacticalOverlay.hudVisible()) return;
