...
public final class ClientCombatState {
    ...
    public static void receiveEncounter(CombatNetwork.EncounterState state, IPayloadContext context) {
        ...
        context.enqueueWork(() -> {
            ...
            if (encounter == null || !encounter.encounterId().equals(state.encounterId()) || encounter.phase() != state.phase() || encounter.active() != state.active())
                DebugDiagnostics.log("client encounter generation={} encounter={} version={} phase={} active={}", state.generation(), state.encounterId(), state.version(), state.phase(), state.active());
    ...
    static void refreshSession() {
        ...
        if (currentConnection == null || currentPlayer == null || currentDimension == null
            ...
            if (connection != currentConnection || playerInstance != minecraft.player || levelInstance != minecraft.level)
                DebugDiagnostics.log("client session reset previousPlayer={} player={} previousDimension={} dimension={}", playerId, currentPlayer, dimension, currentDimension);
            bodyPaused = false;
            ClientControl.reset();
